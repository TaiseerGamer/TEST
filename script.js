...
function renderShop() {
  els.shop.innerHTML = BOXES.map((box) => `
    <button class="box-btn" data-box="${box.id}" ${state.money < box.price ? "disabled" : ""}>
      <span class="box-emoji">${box.emoji}</span>
      <span>
        <strong>${box.name}</strong>
        <small>${box.blurb}</small>
      </span>
...
function openBox(box) {
  ...
  els.reveal.innerHTML = `
    <span class="food">${food.emoji}</span>
    <div>
      <div><strong>${isNew ? "New food!" : "You got"} ${food.name}</strong></div>
      <div class="rarity ${food.rarity}">${food.rarity} • from ${box.name}</div>
    </div>
  `;
